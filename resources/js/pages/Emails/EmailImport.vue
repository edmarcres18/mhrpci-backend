...
const breadcrumbs = [
  ...
  { title: 'Import', href: '/emails/import' },
];
...
async function submit() {
  try {
    ...
    const res = await axios.post('/api/emails/import', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
    if (res.data?.success) {
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="mx-auto w-full max-w-5xl p-4 space-y-6">
      ...
      <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold">Import Emails</h1>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Supported formats: .xlsx, .xls, .csv • Columns: Department, Email, Password, Person In Charge, Position</CardDescription>
        </CardHeader>
