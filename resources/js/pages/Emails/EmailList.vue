...
function exportExcel() { window.location.href = '/api/emails/export/excel'; }
function exportPdf() { window.location.href = '/api/emails/export/pdf'; }
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="mx-auto w-full max-w-7xl p-4 space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div class="space-y-1">
          <h1 class="text-2xl sm:text-3xl font-semibold tracking-tight">Emails</h1>
          <p class="text-sm text-muted-foreground">Manage email records, import/export and generate PDF</p>
        </div>
        <div class="flex items-center gap-2">
          <Link href="/emails/create"><Button class="w-full sm:w-auto"><Plus class="size-4" /> Add</Button></Link>
          <Link href="/emails/import"><Button class="w-full sm:w-auto" variant="secondary"><Upload class="size-4" /> Import</Button></Link>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Search and filter email records</CardDescription>
        </CardHeader>
        <CardContent>
          <div class="grid gap-3 sm:grid-cols-3">
            <div class="relative">
              <Search class="absolute left-2 top-1/2 -translate-y-1/2 size-4 text-muted-foreground" />
              <Input class="pl-8 w-full" v-model="search" placeholder="Search by email, person, position" @input="onSearchInput" />
            </div>
