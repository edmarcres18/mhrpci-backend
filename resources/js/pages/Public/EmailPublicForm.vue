...
function validateLocal(): boolean {
  ...
  const invalidDomains = ['example.com', 'test.com', 'sample.com', 'demo.com', 'localhost', 'test.test'];
  
  if (!form.department?.trim()) map.department = 'Department is required';
  if (!form.email?.trim()) {
    map.email = 'Email is required';
  } else if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(form.email)) {
    map.email = 'Email must be valid';
  } else {
    const domain = form.email.toLowerCase().split('@')[1];
    if (invalidDomains.includes(domain)) {
      map.email = 'Please use a valid company email, not a sample/test email';
    }
  }
  if (!form.password?.trim()) map.password = 'Password is required';
...
async function submit() {
  try {
    ...
    const res = await axios.post('/public/emails', { ...form });
    if (res.data?.success) {
      toast.type = 'success'; toast.message = 'Email submitted successfully'; toast.show = true; window.setTimeout(() => (toast.show = false), 3000);
      Object.assign(form, { department: '', email: '', password: '', person_in_charge: '', position: '', website: '' });
    }
  } catch (e: any) {
...
<template>
  ...
  <div class="public-form">
      ...
      <div v-if="showPurposeModal" class="modal-overlay" @click="closeModal">
        <transition name="zoom">
          <div v-if="showPurposeModal" ref="modalRef" class="modal" role="dialog" aria-modal="true" aria-labelledby="purpose-title" aria-describedby="purpose-desc" @keydown="handleModalKeydown" @click.stop>
            ...
            <p id="purpose-desc" class="modal-text">This form helps keep our inventory of company email accounts current and secure. Share your department’s details to support better access control and monitoring.</p>
          </div>
        </transition>
      </div>
    </transition>
    ...
    <div class="container">
      ...
      <h1 class="title">Enter Company Email Used</h1>
      <p class="subtitle">Provide your department’s official company email currently in use</p>
